...
def home():
    ...
    <label>仓库路径或 GitHub URL</label><input id='repo' value='https://github.com/psf/requests.git' placeholder='例如：D:\\repos\\my-project 或 https://github.com/owner/project.git'>
    <label>问题</label><textarea id='question'>HTTP 异常在哪里定义？请给出文件和行号</textarea><button onclick='run()'>开始分析</button>
    <p class='muted'>示例：<a href='#' onclick="sample('https://github.com/psf/requests.git','HTTP 异常在哪里定义？请给出文件和行号');return false">Requests 异常定义</a> · <a href='#' onclick="sample('https://github.com/fastapi/fastapi.git','请分析路由注册的核心调用链');return false">FastAPI 路由调用链</a></p>
